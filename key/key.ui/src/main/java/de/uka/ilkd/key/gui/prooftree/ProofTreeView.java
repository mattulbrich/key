...
public class ProofTreeView extends JPanel implements TabPanel {
    ...
    @Override public void visualizerUnregistered(NodeInfoVisualizer vis){if(vis.getNode().proof()!=null&&!vis.getNode().proof().isDisposed()&&vis.getNode().proof()==proof){delegateModel.updateTree(vis.getNode());}}

    @Override public void visualizerRegistered(NodeInfoVisualizer vis){delegateModel.updateTree(vis.getNode());}};
    ...
    class GUITreeSelectionListener implements TreeSelectionListener,
        ...
        public void valueChanged(TreeSelectionEvent e) {
            ...
            delegateView.setEditable(treeNode instanceof GUIBranchNode && treeNode
                    .getNode().parent() != null);
        }
    }
    ...
    public class ProofRenderer extends DefaultTreeCellRenderer implements TreeCellRenderer {
        ...
        private void renderLeaf(Style style, GUIAbstractTreeNode node) {
            try {
                ...
                if (goal == null || leaf.isClosed()) {
                    ...
                    tooltip.title = "Linked goal - no automatic rule application";
                    ...
                    tooltip.title = "Interactive goal - no automatic rule application";
                } else {
                    ...
                    tooltip.title = "An open goal";
                }
                if (notes != null) {
                    tooltip.notes = notes;
                }
                var oldTooltip = style.get(KEY_TOOLTIP);
                if (!oldTooltip.isEmpty()) {
                    tooltip.additionalInfo = oldTooltip;
                }
                style.set(KEY_TOOLTIP, tooltip.render());
            } catch (ClassCastException ignored) {
            }
        }
        ...
        private void renderNonLeaf(Style style, GUIAbstractTreeNode treeNode) {
            ...
            var tooltip = new TooltipContent();
            tooltip.title = "An inner node of the proof";
            ...
            if (child != null && child != node && child.getNodeInfo().getBranchLabel() != null) {
                isBranch = true;
                style.set(KEY_TEXT,
            ...
            if (isBranch && node.childrenCount() > 1) {
                ...
                tooltip.title = "A branch node with all children hidden";
            }
            ...
            var text = style.get(KEY_TEXT);
            // Elide text and move it to additional info
            // This does not influence the search since it uses the toString repr
            if (tooltip.additionalInfo.isEmpty() && text.length() > 60
                    && treeNode instanceof GUIProofTreeNode) {
                style.set(KEY_TEXT, text.substring(0, 60) + "...");
                ...
                tooltip.additionalInfo = info == null ? node.name() : info;
            }
            style.set(KEY_TOOLTIP, tooltip.render());
        }
        ...
        private void oneStepSimplification(Style style, GUIAbstractTreeNode node) {
            if (node instanceof GUIOneStepChildTreeNode) {
                ...
                var text = ((GUIOneStepChildTreeNode) node).appliedOn();
                style.set(KEY_TOOLTIP, text);
            }
        }
        ...
        public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel,
            ...
            var tooltip = style.get(KEY_TOOLTIP);
            if (!tooltip.isEmpty()) {
                setToolTipText("<html>" + tooltip + "</html>");
            }
            setIcon(style.get(KEY_ICON));
            setText(style.get(KEY_TEXT));
