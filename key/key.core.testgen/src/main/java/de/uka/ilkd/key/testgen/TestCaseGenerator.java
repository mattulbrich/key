...
public class TestCaseGenerator {
    ...
    private String createCompileWithOpenJML(String openJMLPath, String objenesisPath) {
        return "#!/bin/bash" + NEW_LINE + NEW_LINE
            ...
            + JAVA_FILE_EXTENSION_WITH_DOT + NEW_LINE
            + "   else" + NEW_LINE
    ...
    private String createExecuteWithOpenJML(String path, String objenesisPath) {
        return "#!/bin/bash" + NEW_LINE
            ...
            + "           quit" + NEW_LINE
            + "        else" + NEW_LINE
            + "           java -cp " + objenesisPath + File.separator + OBJENESIS_NAME + ":" + path
            + File.separator + "jmlruntime.jar:" + path + File.separator + "jmlspecs.jar:. $1"
            + NEW_LINE
            + "        fi" + NEW_LINE
            + "      else" + NEW_LINE
            + "         echo \"objenesis-2.2.jar not found!\"" + NEW_LINE
                    ...
                    if (TestCaseGenerator.modelIsOK(m)) {
                        ...
                        if (junitFormat) {
                            testMethod.append("   //calling the test oracle").append(NEW_LINE)
                                    .append(TAB)
                                    .append(oracleMethodCall).append(NEW_LINE);
                        }
                ...
                if (!type.equals("NOTYPE")) {
                    ...
                    if (junitFormat && (isObject || Character.isJavaIdentifierStart(c.charAt(0)))
                        ...
                        if (isLiteral) {
                            ...
                            assignments.add(new Assignment(declType, getPreName(c), "(" + type + ")"
                                + getPreName(val)));
                        }
    ...
    private String createOldMap(Set<String> objNames) {
        StringBuilder result =
            new StringBuilder(NEW_LINE + TAB + "Map<Object,Object> " + OLDMap
                + " = new HashMap<Object,Object>();");
        for (String o : objNames) {
            result.append(NEW_LINE).append(TAB).append(OLDMap).append(".put(")
                    .append(getPreName(o)).append(",").append(o).append(");");
        }
        return result.toString();
    }
    ...
    private String createBoolSet() {
        ...
        return NEW_LINE +
            TAB + "Set<Boolean> " + allbool + "= new HashSet<Boolean>();" + NEW_LINE +
            TAB + allbool + ".add(true);" + NEW_LINE +
            TAB + allbool + ".add(false);" + NEW_LINE;
    }
    ...
    private String createIntSet() {
        ...
        for (long i = low; i <= hi; i++) {
            res.append(TAB).append(allint).append(".add(")
                    .append(i).append(");").append(NEW_LINE);
        }
        return res.toString();
    }
