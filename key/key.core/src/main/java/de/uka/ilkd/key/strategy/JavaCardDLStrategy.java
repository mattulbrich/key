...
public class JavaCardDLStrategy extends AbstractFeatureStrategy {
    ...
    private RuleSetDispatchFeature setupCostComputationF() {
        ...
        if (strategyProperties.contains(StrategyProperties.AUTO_INDUCTION_ON)
            ...
            bindRuleSet(
                d,
                "induction_var",
                ifZero(applyTF(instOf("uSub"), IsInductionVariable.INSTANCE),
    ...
    private boolean autoInductionEnabled() { // chrisg
        // Negated!
        return !StrategyProperties.AUTO_INDUCTION_OFF.equals(strategyProperties
                .getProperty(StrategyProperties.AUTO_INDUCTION_OPTIONS_KEY));
    }
    ...
    private void setupPolySimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        final TermFeature atLeastTwoLCEquation =
            opSub(Equality.EQUALS,
                opSub(tf.mul, tf.atom, tf.atLeastTwoLiteral), tf.intF);
        ...
        bindRuleSet(
            ...
            ifZero(MatchedIfFeature.INSTANCE,
                add(monSmallerThan("cpLeft1", "cpLeft2", numbers),
                    not(TrivialMonomialLCRFeature.create(
                        instOf("cpLeft1"), instOf("cpLeft2"))))));
    }
    ...
    private void setupPolySimpInstantiationWithoutRetry(RuleSetDispatchFeature d) {
        ...
        bindRuleSet(
            ...
            ifZero(not(isInstantiated("newSymDef")), SumFeature.createSum(
                ...
                instantiate("newSymDef", MonomialColumnOp.create(
                    instOf("newSymLeftCoeff"),
                    instOf("newSymRight"))))));
    ...
    private void setupInEqSimp(RuleSetDispatchFeature d, IntegerLDT numbers) {
        ...
        bindRuleSet(d, "inEqSimp_commute", SumFeature.createSum(
        ...
        bindRuleSet(d, "inEqSimp_exactShadow",
            SumFeature.createSum(new Feature[] {
                ...
                ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(
                    applyTFNonStrict("esCoeff1", tf.nonNegLiteral),
                    applyTF("esRight1", tf.polynomial),
                    not(PolynomialValuesCmpFeature.leq(
                        instOf("esRight2"), instOf("esRight1"),
        ...
        bindRuleSet(
            ...
            add(applyTF("subsumLeft", tf.monomial),
                ifZero(MatchedIfFeature.INSTANCE, SumFeature.createSum(
                    ...
                    applyTFNonStrict("subsumCoeffSmaller",
                        tf.posLiteral),
                    applyTFNonStrict("subsumCoeffBigger",
                        tf.posLiteral),
        ...
        bindRuleSet(d, "inEqSimp_or_tautInEqs",
            SumFeature.createSum(new Feature[] {
                ...
                PolynomialValuesCmpFeature.leq(
                    instOf("tautRightSmaller"),
                    opTerm(numbers.getAdd(), one,
                        instOf("tautRightBigger"))) }));
        ...
        bindRuleSet(d, "inEqSimp_or_antiSymm",
            SumFeature.createSum(new Feature[] {
                ...
                PolynomialValuesCmpFeature.eq(
                    opTerm(numbers.getAdd(), two,
                        instOf("antiSymmRightSmaller")),
    ...
    private void setupMultiplyInequations(RuleSetDispatchFeature d,
        ...
        /*
         * final Feature partiallyBounded = not ( sum ( intRel,
         * SequentFormulasGenerator.sequent (), not ( add ( applyTF ( intRel,
         * tf.intRelation ), InEquationMultFeature .partiallyBounded ( instOf (
         * "multLeft" ), instOf ( "multFacLeft" ), sub ( intRel, 0 ) ) ) ) ) );
         */
        ...
        final Feature totallyBounded =
            not(sum(intRel,
                ...
                not(add(applyTF(intRel, tf.intRelation),
                    InEquationMultFeature.totallyBounded(
                        instOf("multLeft"),
                        instOf("multFacLeft"), sub(intRel, 0))))));

        final Feature exactlyBounded =
            not(sum(intRel,
                SequentFormulasGenerator.sequent(),
                not(add(applyTF(intRel, tf.intRelation),
                    InEquationMultFeature.exactlyBounded(
                        instOf("multLeft"),
                        instOf("multFacLeft"), sub(intRel, 0))))));
            ...
            add(applyTF("multLeft", tf.nonNegMonomial),
                ...
                ifZero(MatchedIfFeature.INSTANCE,
                    SumFeature
                             ...
                             */
                            /*
                             * applyTF ( "multLeft", rec ( tf.mulF,
                             * longTermConst ( 20 ) ) ), applyTF (
                             * "multFacLeft", rec ( tf.mulF,
                             * longTermConst ( 20 ) ) ), applyTF (
                             * "multRight", rec ( tf.addF,
                             * longTermConst ( 4 ) ) ), applyTF (
                             * "multFacRight", rec ( tf.addF,
                             * longTermConst ( 4 ) ) ),
                             */
                            ), notAllowedF)));
    }
    ...
    private void setupSquaresAreNonNegative(RuleSetDispatchFeature d) {
        ...
        final Feature productContainsSquare =
            applyTF(sub(product, 0),
                or(eq(factor), opSub(tf.mul, any(), eq(factor))));
    ...
    private Feature isRootInferenceProducer(TermBuffer intRel) {
        return applyTF(intRel,
            add(tf.intRelation, sub(tf.nonCoeffMonomial, tf.literal)));
    }
            ...
            add(isInstantiated("cutFormula"),
                or(not(sum(
                    ...
                    ifZero(applyTF("cutFormula",
                        ...
                        let(atom,
                            sub(instOf("cutFormula"), 0),
                            let(literal,
                                sub(instOf("cutFormula"),
                                    1),
                                allowInEqStrengthening(
                                    atom, literal)))))));
    }
    ...
    private void setupDefOpsExpandMod(RuleSetDispatchFeature d) {
        ...
        final Feature subsumedModulus =
            add(applyTF(superTerm,
                sub(opSub(tf.mod, any(), tf.literal), tf.zeroLiteral)),
                PolynomialValuesCmpFeature.divides(instOf("divDenom"),
                    sub(sub(superTerm, 0), 1)));

        final Feature exSubsumedModulus =
            add(applyTF("divDenom", tf.literal),
                not(sum(superTerm, SuperTermGenerator
                        .upwardsWithIndex(
    ...
    private void setupDivModDivision(RuleSetDispatchFeature d) {
        ...
        final Feature checkNumTerm =
            ifZero(add(not(applyTF(numTerm, tf.addF)),
                ...
                add(instantiate("polyDivCoeff",
                    ReduceMonomialsProjection.create(numTerm,
                        denomLC)),
                    inftyConst()));

        final Feature isReduciblePoly =
            sum(numTerm, SubtermGenerator.rightTraverse(instOf("divNum"),
        ...
        final Feature checkCoeffE =
            ifZero(contains(divCoeff, FocusProjection.create(0)),
                ...
                longConst(0),
                add(instantiate("polyDivCoeff", divCoeff), inftyConst()));
        ...
        final Feature isReduciblePolyE =
            sum(numTerm,
                ...
                ifZero(applyTF(numTerm, tf.addF),
                    longConst(0),
                    sum(divCoeff, MultiplesModEquationsGenerator
    ...
    private void setupClassAxiomApproval(final RuleSetDispatchFeature d) {
        ...
        if (classAxiomApplicationEnabled()) {
            ...
            final Feature needsInstantiation =
                SVNeedsInstantiation.create("sv_heap");
            /*
             * if 'sv_heap' is present and instantiated, allow application only
             * if the heap used to instantiate 'sv_heap' still occurs in the
             * sequent. Otherwise this was a rather short lived heap term which
             * has been rewritten since. Hence, we discard it to avoid too many
             * most likely useless applications.
             */
            final Feature approveInst =
                ifZero(isInstantiated("sv_heap"),
                    /*
                     * the sum expression is 0 if the heap does not
                     * occur and infinite if it does occur, the outer
                     * 'not' then ensures that the costs are infinite in
                     * the first and 0 in the latter case
                     */
    ...
    private void setClassAxiomInstantiation(final RuleSetDispatchFeature d) {
        ...
        final Feature heapInstantiator =
            ifZero(needsInstantiation,
                forEach(heapVar,
