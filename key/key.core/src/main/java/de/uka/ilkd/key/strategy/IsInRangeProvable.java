...
public class IsInRangeProvable implements Feature {
     ...
     */
    private ImmutableSet<Term> collectAxioms(Sequent seq, PosInOccurrence ignore,
            Services services) {
        ...
        final ImmutableSet<Term> result =
            extractAssumptionsFrom(seq.antecedent(), false, DefaultImmutableSet.<Term>nil(), ops,
                formulaToIgnore, services);
    ...
    /**
     * helper method to determine teh formulas in a semisequent to be used as axioms (or their
     * negations)
     *
     * @param semisequent the {@link Semisequent}, i.e., antecedent or succedent from which axioms
     *        are extracted
     * @param negated a boolean true if not the formulas of the sequent itself, but their negation
     *        should be used as axiom
     ...
     * @param formulaToIgnore the {@link SequentFormula} that should not be used as axiom under any
     *        circumstances (the consequence is derived from this formula)
     * @param services the {@link Services}
    ...
    private boolean filterSequent(final HashSet<Operator> ops,
        ...
        return (formula.op() == Equality.EQUALS &&
                formula.sub(0).sort().extendsTrans(integerLDT.targetSort()))
                || ops.contains(formula.op());
    }
