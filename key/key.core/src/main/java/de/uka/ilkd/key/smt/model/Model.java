...
public class Model {
    ...
    private void processLocSetNames() {
        ...
        SMTSort locsetSort = this.getTypes()
                .getSort(SMTObjTranslator.LOCSET_SORT);
        SMTSort objectSort = this.getTypes()
                .getSort(SMTObjTranslator.OBJECT_SORT);
        SMTSort fieldSort = this.getTypes()
                .getSort(SMTObjTranslator.FIELD_SORT);
        // for all locsets
     ...
     * @param s
     *        sort of constant
     * @return human readable form #sn with s the first letter of the sort of
    ...
    public String processAnyValue(String val) {
        ...
        /*
         * val now contains the binary string, we check the type bits for all
         * possible sort types
         */

        // val is of type bint
        if (val.startsWith(types
                .getPrefixForSort(types.getSort(SMTObjTranslator.BINT_SORT)))) {
            long intBound = types.getSort(SMTObjTranslator.BINT_SORT)
                    .getBound();
            long intSize = types.getSort(SMTObjTranslator.BINT_SORT)
                    .getBitSize();
            val = val.substring(3);
